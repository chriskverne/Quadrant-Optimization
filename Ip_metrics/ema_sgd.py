...
def train_qnn_param_shift(x, y, n_qubits, n_layers, num_measurment_gates, num_epochs):
    ...
    # --- CHANGE 1: Renamed variable and added beta for EMA ---
    # Tracks EMA of gradients to decide what to freeze
    ema_grads = pnp.zeros_like(params)
    beta = 0.9 # Decay factor for EMA
    ...
    for epoch in tqdm(range(num_epochs), desc="Epochs"):
        ...
        for image, label in tqdm(zip(x_t, y_t), total=len(x_t), desc=f"Epoch {epoch+1}/{num_epochs}", leave=False):
            ...
            # --- CHANGE 2: Updated importance score from sum to EMA ---
            # Update the Exponential Moving Average of gradients
            ema_grads = beta * ema_grads + (1 - beta) * gradients
        ...
        # --- CHANGE 3: Use EMA grads for freezing decision ---
        # Decide what to freeze based on the magnitude of the EMA
        flat_grads = pnp.abs(ema_grads.flatten())
        ...
        # --- CHANGE 4: Reset EMA for active params, keep for frozen ---
        # Frozen params (0) keep their historical EMA, active params (1) reset to 0
        ema_grads = pnp.where(active_p == 0, ema_grads, 0)  
